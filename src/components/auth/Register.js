...
export const Register = () => {
        ...
        fetch("http://localhost:8088/users", {
            ...
            .then(createdUser => {
                if (createdUser.hasOwnProperty("id")) {
                    localStorage.setItem("mybrary_user", JSON.stringify({
                        id: createdUser.id,
        ...
        .then((res) => res.json())
            .then(response => {
                if (response.length > 0) {
                    ...
                    <Alert variant="danger">Account with that username already exists</Alert>;
                }
                else {
    ...
    const updateUser = (evt) => {
      ...
      return (
        ...
        <a href="/" className="link-success register">Sign In</a>
        </div>
        </div>
