...
export const SearchBar = ({setterFunction}) => {
  ...
  const fetchMethod = (e) => {
    e.preventDefault();
    fetch(`https://www.googleapis.com/books/v1/volumes?q=${books.searchField}&maxResults=40`)
    ...
    const cleanedData = books.map((book) => {
      if (book.volumeInfo.hasOwnProperty('publishedDate') === false) {
        ...
        if (book.volumeInfo.hasOwnProperty('imageLinks') === false) {
          book.volumeInfo['imageLinks'] = { smallThumbnail: 'https://icon-library.com/images/no-image-available-icon/no-image-available-icon-10.jpg'}
        }
      }
      return book
    })
